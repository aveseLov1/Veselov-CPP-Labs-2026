...
//     explicit Fraction(const char* str);  // конструктор преобразования из строки
//     Fraction(const Fraction& other);     // конструктор копирования

//     // Конструкторы для преобразования простых типов (Часть 2)
//     explicit Fraction(double value);     // из double
...
//     // Арифметические операторы (только для дробей)
